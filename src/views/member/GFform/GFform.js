  ...
  for(i in titles){
	  ...
	  for(var j in GFs){
		  if(selected[i].includes(parseInt(j))){
			  GF_options.push({"value":GFs[j].name, "label": (<span style={{whiteSpace:"pre"}}><b>{GFs[j].name}</b></span>), "id": j});
			  selected_options.push({"value":GFs[j].name, "label": (<span style={{whiteSpace:"pre"}}><b>{GFs[j].name}</b></span>), "id": j});
		  }
		  else GF_options.push({"value":GFs[j].name, "label": (<span style={{whiteSpace:"pre"}}><b>{GFs[j].name}</b> <span>{"      " + GFs[j].note}</span></span>), "id": j});
	  }
	  inputs.push(<CFormGroup>
		    <CLabel>{titles[i]}</CLabel>
		  	<Select value={selected_options} isMulti autoFocus options={GF_options} onChange={function(set_selected, selected, i, value){
				var tmp = [];
				for(let v of value){
    ...
    <CRow>
	  <CCol>
		<CCard>
		  ...
		  <CCardFooter>
			<CButton variant="ghost" color="primary" onClick={function(setAddModal){setAddModal(true)}.bind(null, setAddModal)}>新增牧養對象</CButton>
